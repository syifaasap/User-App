...
class _MyMainScreenState extends State<MyMainScreen> {
  ...
  double searchLocationContainerHeight = 600;
...
// Apabila izin ditolak
  ...
  Future hitTelegram() async {
    print("Button Pressed");
    var botToken = '6276588282:AAFu66nqwjfZXuMePuIQrFM9VfZbd0yQOZ8';
    final username = (await Telegram(botToken).getMe()).username;
    var teledart = TeleDart(botToken, Event(username!));

    teledart.stop();
    teledart.start();
    print("BOT telegram initialized : $username");
    // ignore: unnecessary_set_literal
    teledart.onMessage().listen((message) => ({
          message.reply('Reply from flutter app, you send : ${message.text}'),
          print(message.text),
        }));
  }
  ...
  showUIForAssignedTechnicianInfo() {
    setState(() {
      waitingResponseFromTechnicianContainerHeight = 240;
